...
extension UserDefaults {
    private enum Keys {
        static let bingoGame = "bingoGame"
    }
    
    func saveBingoGame(_ game: BingoGame) {
        let encoder = JSONEncoder()
        if let encoded = try? encoder.encode(game) {
            set(encoded, forKey: Keys.bingoGame)
        }
    }
    
    func loadBingoGame() -> BingoGame? {
        if let savedGameData = data(forKey: Keys.bingoGame) {
            let decoder = JSONDecoder()
            if let loadedGame = try? decoder.decode(BingoGame.self, from: savedGameData) {
                return loadedGame
            }
        }
        return nil
